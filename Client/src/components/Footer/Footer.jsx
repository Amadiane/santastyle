...
// ✅ Charte alignée sur BoutiquePage / NosMissions / Contact
const SS = {
  bg:        "#FFFFFF",
  surface:   "#FDFAF4",
  card:      "#F7F2E8",
...
// ✅ Numéro format international correct pour api.whatsapp.com
...
const Footer = () => {
  ...
  // ✅ Boutique : 4 genres + nouveautés (sans pays)
  const boutique = [
  ...
  // ✅ Boutons WA footer — sans émojis complexes, messages propres
  const waButtons = [
  ...
  return (
    <>
      ...
      {/* ── Toast ── */}
      {toast && (
        <div style={{ position: "fixed", top: "24px", right: "24px", zIndex: 9999, maxWidth: "360px", width: "calc(100% - 48px)", animation: "fadeUp 0.3s ease" }}>
      ...
      >
        {/* Déco fond — points dorés discrets */}
        ...
        <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "56px 24px 0", position: "relative" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "48px", marginBottom: "48px", paddingBottom: "48px", borderBottom: `1px solid ${SS.border}` }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
              ...
              <div>
                <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "12px" }}>
                  <div style={{ width: "44px", height: "44px", borderRadius: "11px", background: `linear-gradient(135deg, ${SS.goldDark}, ${SS.gold})`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 4px 16px ${SS.gold}35` }}>
                    <span style={{ color: "#fff", fontSize: "22px", fontWeight: "900", fontFamily: "serif" }}>S</span>
                  </div>
                  <div>
                    <div style={{ fontSize: "20px", fontWeight: "800", color: SS.goldDark, letterSpacing: "-0.02em" }}>Santa'Style</div>
                    <div style={{ fontSize: "12px", color: SS.textMuted, fontWeight: "500" }}>Inspirer · Porter · Rayonner</div>
                  </div>
                </div>
                {/* ✅ Description sans pays */}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                {contacts.map((c, i) => {
                  const Icon = c.icon;
                  const inner = (
                    <div style={{ display: "flex", alignItems: "center", gap: "12px", padding: "10px 14px", borderRadius: "10px", background: SS.surface, border: `1px solid ${SS.border}`, transition: "all 0.15s" }}
            ...
            <div style={{ display: "flex", flexDirection: "column", justifyContent: "center" }}>
              <div style={{ background: SS.surface, border: `1px solid ${SS.gold}35`, borderRadius: "16px", padding: "28px", position: "relative", overflow: "hidden" }}>
                ...
                <form onSubmit={handleSubscribe} style={{ display: "flex" }}>
                  <input type="email" value={email} placeholder="votre@email.com"
                    ...
                    style={{ flex: 1, padding: "11px 14px", background: "#fff", border: `1px solid ${SS.border}`, borderRight: "none", borderRadius: "10px 0 0 10px", color: SS.text, fontSize: "13px", outline: "none", transition: "border-color 0.2s" }}
                    onFocus={e => e.target.style.borderColor = SS.gold}
                    onBlur={e => e.target.style.borderColor = SS.border} />
                  <button type="submit" disabled={submitting}
                    style={{ padding: "11px 18px", borderRadius: "0 10px 10px 0", background: `linear-gradient(135deg, ${SS.goldDark}, ${SS.gold})`, border: "none", color: "#fff", fontWeight: "700", fontSize: "13px", cursor: submitting ? "not-allowed" : "pointer", opacity: submitting ? 0.6 : 1, display: "flex", alignItems: "center", gap: "6px", whiteSpace: "nowrap" }}>
                    {submitting
                ...
                {/* ✅ Bouton WA — même fonction corrigée */}
                <button onClick={() => ouvrirWA("Bonjour Santa'Style !\nJe souhaite recevoir vos nouveautes et promotions.")}
                  style={{ marginTop: "12px", width: "100%", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", padding: "11px", borderRadius: "10px", border: `1px solid ${SS.success}35`, background: SS.successBg, color: SS.success, fontSize: "13px", fontWeight: "700", cursor: "pointer", transition: "opacity 0.15s" }}
                  onMouseEnter={e => e.currentTarget.style.opacity = "0.85"}
                  onMouseLeave={e => e.currentTarget.style.opacity = "1"}>
                  <WAIcon size={15} color={SS.success} /> Nous contacter sur WhatsApp
                </button>
              </div>
            </div>
          </div>
              ...
              <ul style={{ listStyle: "none", padding: 0, margin: 0, display: "flex", flexDirection: "column", gap: "8px" }}>
                {boutique.map((b, i) => {
                  ...
                  return (
                    <li key={i}>
                      <NavLink to={b.path}
                        style={({ isActive }) => ({ display: "flex", alignItems: "center", gap: "8px", textDecoration: "none", fontSize: "13px", fontWeight: "500", color: isActive ? SS.gold : SS.textMuted, transition: "all 0.15s" })}
              ...
              <div style={{ background: SS.surface, border: `1px solid ${SS.border}`, borderRadius: "12px", padding: "16px", display: "flex", flexDirection: "column", gap: "10px" }}>
                ...
                {waButtons.map((btn, i) => (
                  <button key={i} onClick={() => ouvrirWA(btn.msg)}
                    style={{ display: "flex", alignItems: "center", gap: "8px", padding: "9px 12px", borderRadius: "8px", border: `1px solid ${i === 0 ? SS.success : SS.border}40`, background: i === 0 ? SS.successBg : "#fff", color: i === 0 ? SS.success : SS.textMuted, fontSize: "12px", fontWeight: "600", cursor: "pointer", width: "100%", transition: "opacity 0.15s" }}
